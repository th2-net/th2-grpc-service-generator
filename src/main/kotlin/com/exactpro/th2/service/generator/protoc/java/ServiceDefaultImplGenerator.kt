...
class ServiceDefaultImplGenerator : AbstractJavaServiceGenerator(), Generator {
    ...
    override fun generateForService(
    ...
    private fun generateBlockingDefaultImpl(
        service: ServiceDescriptorProto,
        javaPackage: String,
        messageNameToJavaPackage: Map<String, String>
    ): FileSpec {
        val javaFile = service.methodList.map {
            ...
            MethodSpec.methodBuilder(methodName)
                .addModifiers(PUBLIC)
                .returns(wrapStreaming(createType(it.inputType, messageNameToJavaPackage), it))
            ...
            createClass(
                getBlockingDefaultImplName(name),
                ClassName.get(javaPackage, getBlockingServiceName(name)),
                javaPackage,
                name,
                true,
                it
    ...
    private fun generateAsyncDefaultImpl(
        service: ServiceDescriptorProto,
        javaPackage: String,
        messageNameToJavaPackage: Map<String, String>
    ) : FileSpec {
        val javaFile = service.methodList.map {
            ...
            val name = service.name
            createClass(
                getAsyncDefaultImplName(name),
                ClassName.get(javaPackage, getAsyncServiceName(name)),
                javaPackage,
                name,
                false,
                it
    ...
    private fun createClass(
        ...
        val stubClassName = if (blocking) getBlockingStubClassName(javaPackage, protoName) else getAsyncStubClassName(
            javaPackage,
            protoName
        )
        ...
        return TypeSpec
            ...
            .addMethod(
                MethodSpec.constructorBuilder().addModifiers(PUBLIC).addCode(
                    """
                super();
            """.trimIndent()
                )
            ...
            .addMethod(
                MethodSpec
                        ...
                        .addParameter(
                            ParameterizedTypeName.get(ClassName.get(StubStorage::class.java), stubClassName),
    ...
    private fun createCreateStubMethod(javaPackage: String, protoName: String, blocking: Boolean) = MethodSpec
        ...
        .returns(
            if (blocking) getBlockingStubClassName(javaPackage, protoName) else getAsyncStubClassName(
                javaPackage,
                protoName
            )
        )
        ...
        .addCode(
            CodeBlock.of(
                ...
                }.${if (blocking) "newBlockingStub" else "newStub"}(channel);"
            )
        )
        .build()
}